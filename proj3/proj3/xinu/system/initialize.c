...
static	void	sysinit()
{
	...
	for (i = 0; i < NPROC; i++) {
		...
		prptr->runtime=0;
		prptr->prpark=0;
		prptr->prioinit=prptr->prprio;
		//prptr->pr_locks=newqueue();
		//prptr->pr_locks[NPILOCKS]={0};
		prptr->pr_tot_locks=0;
		prptr->deadlock_det=0;
	}
